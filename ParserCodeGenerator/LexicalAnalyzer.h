...
char* tokenToTokenRep(int token);
	...
	for (i = 0; i< token_cnt; i++)
	{
		...
		if (tokens[i] == numbersym)
		{
			for (j = 0; j<cnt; j++) 
			{
				...
				{
					fprintf(ofp, "%d %d ", tokens[i], num[num_link]);

					if (print)
						printf("%d %d ", tokens[i], num[num_link]);
		...
		for (i = 0; i< token_cnt; i++)
		{
			...
			if (tokens[i] == numbersym)
			{
				for (j = 0; j<cnt; j++)
				{
					if (ST[j].val == tokens[i + 1])
					{
						printf("%s %d ", "numbersym", num[num_link]);
						cnt++;
						num_link++;
						num_print++;
						symflag = 1;
			...
			{
					printf("%s ", tokenToTokenRep(tokens[i]));
			}
		}
	}
